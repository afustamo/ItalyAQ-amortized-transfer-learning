...
total_sims <- 20000
# chunks the data will be created in (RAM constraints)
# 3000 needs about 16gb, 2000 is quite safe
chunk_size <- 2000
...
generate_i2i_data <- function(
  ...
  script_time <- system.time(
    for (sim in 1:N_SIMS){
      ...
      f <- LKrig.sim( sGrid, LKinfo, M = n_replicates) # using basis functions here 
      mu <- rowMeans(f)
      sd <- apply(f, 1, sd) 
...
# cranking it up from 2 to 10 here
awghts_log <- 4 + exp(seq(log(0.0001),
                          log(10),
                          length.out=n_awghts_log))
awghts_unif <- 4 + seq(0.0001, 10, length.out=n_awght_unif)
