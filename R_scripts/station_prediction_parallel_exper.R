...
parallel::detectCores(logical = TRUE)   # logical (threads)

n_cores <- 12 # for Antony's laptop (as it has 24 physical)
...
proj_dir <- getwd()  # remember project dir for workers
...
est_additional_kappa2 <- function(
  ...
  nonstat_Q <- LKrig.precision(LKinfo)
...
clusterExport(cl, c(
  "make_nonstat_LKinfo",
  "est_additional_kappa2",
  "predict.multivariateSurfaceGrid",
  "gridList_small",
  "sGrid_small",
  "stations_df",
  "covars",
  ...
  "LatticeKrig_hacked", 
  "LKrig_hacked", 
  "LKrigFindLambdaAwght_hacked", 
  "LambdaAwghtObjectiveFunction_hacked"
))
registerDoParallel(cl)
# clusterEvalQ(cl, setwd(proj_dir))

# ------------------------------------
# Experiment Loop (parallel over days)
# ------------------------------------

tic()

cv_results_mat <- foreach(
  day_idx = 1:n_days,
  .combine = rbind,
  .packages = c("rhdf5", "LatticeKrig", "sf", "fields")
) %dopar% {
  ...
  if (n_obs < cv_type * 2) {
    ...
    # return row of NAs for this day
    return(c(day_idx, NA, NA, NA, NA))
  }
  ...
  # ------------------------------------
  lm_rmse       <- mean(rmse_lm_vec,       na.rm = TRUE)
  stat_rmse     <- mean(rmse_stat_vec,     na.rm = TRUE)
  ns_rmse       <- mean(rmse_ns_vec,       na.rm = TRUE)
  ns_k2add_rmse <- mean(rmse_ns_k2add_vec, na.rm = TRUE)
  
  c(day_idx, lm_rmse, stat_rmse, ns_rmse, ns_k2add_rmse)
}
...
cv_results_df <- as.data.frame(cv_results_mat)
names(cv_results_df) <- c("day", "lm_rmse", "stat_rmse", "ns_rmse", "ns_k2add_rmse")
