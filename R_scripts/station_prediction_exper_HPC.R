...
## --> set the environment variable AQ_amortised_HPC = "TRUE" <-- ##
...
CV_type <- "10-fold"
dir.create("data/output/map", recursive = TRUE)
dir.create("data/output/coef", recursive = TRUE)
...
params_all_surr30 <- h5read(here("results", "STUN_param_df_2023padded"),
...
stations_df <- merge(stations_df, Station_registry_information[, c("AirQualityStation", "AirQualityStationType")])
...
foreach(
  ...
  if (N_data >= 250) {
    if (surfaces) {
      ...
      ## 2c. LK_stun - Lattice Kriging + STUN parameters ####
      ...
      ## 2d. LK_stun_adj - Adjusted Lattice Krig + STUN ####
      ...
      sp_param <- cbind(
        sp_param,
        data.frame(
          kappa2_adj = kappa2_adj,
          kappa2_stun = kappa2_stun,
          kappa2_eea = kappa2_eea
        )
      )  # this is the kappa2 parameter that controls the spatial correlation in the model. The larger it is, the less spatial correlation
      ...
      # nugget effect
      err_param <- cbind(err_param,
                         data.frame(tau_stun_adj = LK_stun_adj$tau.MLE.FULL)) # this is the nugget parameter that controls the amount of measurement error in the model. The larger it is, the more measurement error there is.
      err_param <- cbind(err_param,
                         data.frame(sigma2_stun_adj = LK_stun_adj$sigma2.MLE.FULL)) # this is the sigma2 parameter that controls the amount of spatial variability in the model. The larger it is, the more spatial variability there is.
      
      # surfaces
      if (!only_params) {
        pred_sp_day@data$LK_stun_adj <- predict(LK_stun_adj, s_pred, Z = Z_pred)[, 1]
        pred_sp_day@data$LK_stun_adj_ls <- predict(
          LK_stun_adj,
          xnew = s_pred,
          Z = Z_pred,
          just.fixed = TRUE
      ...
      # 3. exporting ####
      if (!only_params) {
        save(pred_sp_day, file = file.path(
          "data/map",
          paste0("prediction_map_", time_cycle, ".rda")
        )) #10 MB
      }
      ...
      save(coef_ls,
           sp_param,
           err_param,
           file = file.path("data/coef", paste0("coef_", time_cycle, ".rda")))
    ...
    if (CV_scheme) {
      ...
      for (nk in 1:length(kfold)) {
        ...
        ## 2b. LK_def - Lattice Krig Default ####
        ...
        ## 2d. LK_stun - Lattice Kriging + STUN parameters ####
        names_param <- c("surr30", "surr1", "rscale30", "rscale1")
        # import STUN parameters
        params <- params_all_surr30[, 128:1, , time_cycle]
        kappa2 <- exp(params[, , 1])
        awght <- kappa2 + 4
        theta <- params[, , 2] + pi / 2
        rho   <- params[, , 3]
      ...
      # CV_folder
      saveRDS(all_y_test_df, file = file.path(CV_folder, paste0("y_test_df_", time_cycle, ".rds"))) # 7.8 KB
    }
  }
}
